...
## E. Round 3 — from the 12:07 screenshot

Cubes (agents) are solid now with faces — good. Four left:

### E1 — Blue orchestrator hexes (and tri subs) still see-through — winding/culling, not transparency
The magenta **cubes** (`SCNBox`) render solid; the blue **hex prisms** and **tri prisms** (`regularPrism`) do not. `SCNBox` has correct outward winding; the custom prism body triangles have inconsistent winding, so with `isDoubleSided = false` the front walls get **back-face culled** and you see straight through to the interior/back edges. Same will affect the octahedron body (`octahedronWithFrontFace`).

**Fix (pick one):**
- Quick: set the **body** material `isDoubleSided = true` for the prism + octa geometries (it's opaque and writes depth, so no see-through cost). One-liner.
- Correct: fix triangle index winding in `regularPrism` side walls + caps (and the octa `bodyOnly`) so all outward normals point out, then keep single-sided.
Do this to the *body* material only — leave the info-face wall single-sided so its back doesn't show.

### E2 — Human still has no glow
Bloom isn't catching the amber. Bump it: octa edge `emission` → `amber @ ~0.7`, `youHalo` material `emission` → `amber @ ~0.6` (and make the halo a bit larger/brighter), and lower `camera.bloomThreshold` to ~**0.45** with `bloomIntensity` ~**0.35**. The halo tube must be `isDoubleSided`, `writesToDepthBuffer = false` (already) so it reads as a soft disc. YOU should be the most obviously glowing thing on the map.

### E3 — Flow packets move too fast
`connect(...)` uses `travel = human ? 1.1 : 1.6` seconds for the full link. The prototype moves a packet at ~0.32 of the link per second → **~3 s** end to end. Set `travel ≈ 3.0` (human ~2.6). Keep the fade-out/rewind so there's no snap.

### E4 — Active ground ring should pulse *in sync with the bob* (glow on the way down)
Right now the ring opacity runs on its own `sin(phaseT*2.4 + phase)`, independent of the bob (`sin(phaseT*1.7 + phase)`), so they drift out of phase. Tie the ring to the bob's **downward motion**: use the *same* argument as the bob and drive glow from its descent.

```swift
let bobArg = phaseT * 1.7 + Double(phase)          // identical to the bob
let descending = max(0, -cos(bobArg))              // 1 at fastest fall, 0 while rising
let a = 0.12 + 0.6 * descending
mat.diffuse.contents  = col.withAlphaComponent(CGFloat(a))
mat.emission.contents = col.withAlphaComponent(CGFloat(a))   // emission so it blooms
```
Result: the ring flares as the shape drops toward the plane and goes dark as it lifts — reads like an impact/energy pulse. (Bob is `sin(bobArg)`, so velocity ∝ `cos(bobArg)`; descending = negative velocity.)

### Round-3 priority
1. **E1** double-side/fix-winding the prism + octa bodies — kills the blue see-through.
2. E4 bob-synced ring glow.
3. E3 slower packets.
4. E2 human glow (with the E1 body fix + lower bloom threshold).
