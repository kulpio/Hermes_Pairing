...
final class PanelController: NSObject {
    ...
    private func buildWindow() {
        ...
        y -= 14
        content.addSubview(Self.label("Hub = Hermes (Options: Save Team, brief, project root). Workers = Front / Kill / Perms + colors.",
            frame: NSRect(x: PAD, y: y, width: W - 2 * PAD, height: 13), size: 9, secondary: true))
    ...
    private func rebuildList(_ pairs: [String]) {
        ...
        for name in pairs {
            ...
            est += 64 + CGFloat(ws.count) * 34 + 40 + 12
        }
        ...
        for (name, entry, ws, hermesTitle, hCols) in prepared {
            ...
            // Two-line hub: line 1 = identity + Front/Hide/Kill, line 2 = a
            // full-size Options button that can never clip at panel width.
            let hermesH: CGFloat = 64
            y -= hermesH
            ...
            hermesRow.alphaValue = stowed ? 0.55 : 1

            hermesRow.addSubview(swatchButton(
                color: hNS.hi,
                frame: NSRect(x: 8, y: 39, width: 14, height: 14),
                id: name,
            ...
            hermesRow.addSubview(button("Front", #selector(frontPressed(_:)),
                NSRect(x: 228, y: 33, width: 52, height: 26), id: name))
            hermesRow.addSubview(button(stowed ? "Show" : "Hide", #selector(hidePressed(_:)),
                NSRect(x: 284, y: 33, width: 52, height: 26), id: name))
            hermesRow.addSubview(button("Kill", #selector(killPressed(_:)),
                NSRect(x: 340, y: 33, width: 46, height: 26), id: name))
            hermesRow.addSubview(button("Options", #selector(teamOptionsPressed(_:)),
                NSRect(x: 8, y: 4, width: 116, height: 26), id: name))
            hermesRow.addSubview(Self.label("brief · project root · save team · windows",
                frame: NSRect(x: 132, y: 9, width: boxW - 140, height: 14), size: 10, secondary: true))
            ...
            // Activity strip: team status + last claim from the session's
            // bridge artifacts. Refreshes with the panel; viewer is on demand.
            let act = TeamActivity.info(for: name)
            let actH: CGFloat = 40
            y -= actH
            let actRow = NSView(frame: NSRect(x: 0, y: y, width: boxW, height: actH))
            let statusLine = "Activity · \(act.status)" + (act.sentAge.isEmpty ? "" : " · sent \(act.sentAge)")
            actRow.addSubview(Self.label(statusLine,
                frame: NSRect(x: 28, y: 21, width: 230, height: 14), size: 10, secondary: true))
            actRow.addSubview(Self.label(act.claim.isEmpty ? "no claim captured yet" : act.claim,
                frame: NSRect(x: 28, y: 4, width: boxW - 36, height: 14), size: 10, secondary: true))
            actRow.addSubview(button("Reply", #selector(openReplyPressed(_:)),
                NSRect(x: 252, y: 11, width: 60, height: 24), id: name))
            actRow.addSubview(button("Sent", #selector(openSentPressed(_:)),
                NSRect(x: 316, y: 11, width: 52, height: 24), id: name))
            listContainer.addSubview(actRow)
            y -= 10
        }
